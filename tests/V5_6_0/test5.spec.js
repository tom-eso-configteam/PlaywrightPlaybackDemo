...
test('Perform Adhoc IM PI Regression', async ({ page}) => {
  ...
  await page.getByText('02 - Internal').click();
  await page.getByText('NEP_PI_ADHOC_IM').click();
  ...
  //Enters the quantity into the box checkcks the quantity has been inputted correctly
  const quantityInput = page.locator('[id="__jsview2--oPage3000-in-MENGE_IN-inner"]');
